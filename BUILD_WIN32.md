...
* Create a base directory for all projects.  I'm putting everything in 
  `D:\PTS`, you can use whatever you like.  In several of the batch files 
...
* Dowload CMake
  ...
  looks something like `D:\PTS\cmake-2.8.12.2-win32-x86`.  Rename this 
  directory to `D:\PTS\CMake`.

  If you already have CMake installed elsewhere on your system you can use it, 
  but PTS has a few batch files that expect it to be in the base 
...
* Download library dependencies:
   ...
   and should be unpacked into BITSHARES_ROOT directory (ie: D:\PTS).
   Now the package contains: BerkeleyDB, boost 1.55, OpenSSL 1.0.1g.

 * BerkeleyDB

   PTS depends on BerkeleyDB 12c Release 1 (12.1.6.0.20).  You can build 
   this from source or download our pre-built binaries to speed things up.

 * Boost
 
   PTS depends on the Boost libraries version 1.55 or later (I assume 
   ...
   * unzip it to the base directory `D:\PTS`. 
   * This will create a directory like `D:\PTS\boost_1_55_0`.
   
 * OpenSSL

   PTS depends on OpenSSL, and you must build this from source.
    * download the latest OpenSSL source from http://www.openssl.org/source/
    * Untar it to the base directory `D:\PTS`
    * this will create a directory like `D:\PTS\openssl-1.0.1g`.
...
```
  The file `D:\PTS\PTS\libraries\fc\CMakeLists.txt` has the 
  `FIND_PACKAGE(Boost ...)`
...
```
  ...
  debug version of PTS.

* Build OpenSSL DLLs
  ```
cd D:\PTS\openssl-1.0.1g
perl Configure --openssldir=D:\PTS\OpenSSL VC-WIN32
ms\do_ms.bat
nmake -f ms\ntdll.mak
nmake -f ms\ntdll.mak install
```
  This will create the directory `D:\PTS\OpenSSL` with the libraries, DLLs,
  and header files.

#### Build project files for PTS ####
 ...
 * Where is the source code: `D:\PTS\PTS`
 * Where to build the binaries: `D:\PTS\bin` (or bin64 for 64bit development).
...
> cd build
> cmake -G "Visual Studio 12" -T "v120_xp" -DINCLUDE_QT_WALLET ..\PTS
...
* Launch *Visual Studio* and load `D:\PTS\bin\PTS.sln` bin64 directory for 64 bit builds.
...
copy all of the necessary files into your `D:\PTS\install`
